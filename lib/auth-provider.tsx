...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    if ((email === 'admin@gmail.com' || email === 'admin') && password === '12345678') {
      setIsAuthenticated(true)
      setUser({ email: 'admin', name: 'Ferdous Hasan' })
      localStorage.setItem('userEmail', 'admin@gmail.com')
      localStorage.setItem('userName', 'Ferdous Hasan')
      localStorage.setItem('userProfile', JSON.stringify({ email: 'admin@gmail.com', name: 'Ferdous Hasan' }))
